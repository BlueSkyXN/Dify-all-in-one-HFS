...
7. [Hugging Face Abuse Avoidance](./hf-abuse-avoidance.md): 避免 Space abuse-handler 风控、Web terminal 风险、发布前自检和触发后处理。
8. [Release Checklist](./release-checklist.md): GitHub PR、`main`、Hugging Face runtime 和 smoke 的证据记录模板。
9. [Bucket-Lite Persistence Drill](./bucket-lite-drill.md): `/persist`、PostgreSQL fallback、dump backup 和重启保留的演练模板。
10. [Operations Runbook](./ops-runbook.md): `/_ops` 诊断入口、健康检查、日志查看、502 排障和发布验收。
11. [Development Guide](./development.md): 本地开发、改动点、验证命令、提交前检查和常见修改流程。
12. [File Reference](./file-reference.md): 仓库内每个受版本控制文件的职责说明。
13. [Security Notes](./security.md): 演示环境安全边界、默认密钥、公开 Space 风险和生产化注意事项。
