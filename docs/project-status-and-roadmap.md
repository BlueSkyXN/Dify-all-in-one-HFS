...
- 单容器多进程：`supervisord` 管理 Dify Web、API、Worker、Beat、Plugin Daemon、Sandbox、PostgreSQL、Redis、ops-service、admin-service 和 Nginx。
...
该快照覆盖 public health、iframe headers、`/_admin/` 默认关闭、setup/init API，以及带 Space `OPS_TOKEN` 的 `/_ops/health`、`/_ops/system`、`/_ops/metrics`、`/_ops/errors`。若后续是 docs-only 发布，仍要至少确认 GitHub/HF head、`runtime.stage`、`runtime.raw.sha` 和无需 token 的 public smoke；没有 `OPS_TOKEN` 时，不要声称 token-protected `/_ops/*` 已完成线上验证。
...
5. P2 继续增强 `/_ops`：增加版本漂移提示、日志过滤、Plugin Daemon schema 只读检查或 warmup 状态。
