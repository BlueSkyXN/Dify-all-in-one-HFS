...
`dify-all-in-one` 是面向 Hugging Face Docker Space 的 Dify 单容器 Demo 工程。它把 Dify Web/API/Worker/Beat、Plugin Daemon、Sandbox、PostgreSQL、Redis、Nginx、只读 `ops-service` 和默认关闭的 `/_admin` 管理面收敛到一个容器中，用于企业内训、课程演示、PoC 和快速功能验证。
...
- 不要假设 Git remote 名称永远固定。本机可能用 `hf` 指向 Hugging Face Space、`origin` 指向 GitHub remote，也可能用其他命名；push、merge 或对比前先看 `git remote -v`。
...
| `hfs-dev.toml` | HFS v2 最小语义登记：Pattern A、artifact 车道、Space、键名和下载面关系 | No | 修改 HFS 范式分类、目录主权、runtime 获取模式或 Settings key registry 时 |
...
| `.github/workflows/` | GitHub Actions 轻量静态检查 | No | 修改 PR/main gate、权限、触发条件或 `scripts/static-check.sh` 调用方式时 |
| `docker/` | runtime 合同：entrypoint、env、Supervisor、Nginx、ops/admin、PostgreSQL backup、healthcheck | Yes | 修改 `docker/` 下任何文件前 |
...
| `local/` | 受版本控制的研究、计划和能力地图材料；不进 Docker build context | No | 除非用户明确要求，否则不要编辑、清理或当作 generated cache |
...
仓库没有 `package.json`、`pyproject.toml`、`Makefile` 或 workspace package manager。真实命令面来自 shell 脚本、Docker、Python 语法检查、curl smoke 和可选 HF CLI。
...
| `bash -n docker/entrypoint.sh docker/with-dify-env docker/with-plugin-env docker/with-sandbox-env docker/run-dify-agent docker/run-shellctl docker/wait-for-core docker/healthcheck.sh docker/postgres-backup-loop scripts/build.sh scripts/run-demo.sh scripts/admin-smoke.sh scripts/hf-space-smoke.sh scripts/validate-hfs-contract.sh scripts/static-check.sh` | 检查所有 runtime/helper shell 脚本语法 | `docker/`, `scripts/` | 本地 shell 可跑；不需要 Docker 或网络 |
| `scripts/validate-hfs-contract.sh` | 检查 Pattern A / manifest-first artifact HFS 契约：registry、端口、bootstrap、ignore boundary、archive self-test 和 smoke 覆盖 | repo | 本地 shell/Python 可跑；不需要 Docker 或网络 |
...
- 保持 HF Docker Space 约束：单容器、单公开 app port、Nginx 监听 `7860`、运行时持久化数据在 `/data` 或明确的 persistence mount 下。
- `README.md` 的 `app_port: 7860`、`docker/nginx.conf` 的 `listen 7860`、`Dockerfile EXPOSE 7860`、docs 和 Space 设置必须保持一致。
- runtime 尽量以 UID `1000` 的 `user` 运行。不要新增依赖 root runtime 的启动逻辑，除非 `Dockerfile` 明确提供。
- `/data` 是 runtime persistence 边界。不要把 PostgreSQL、Redis、插件缓存、上传文件、日志、generated secrets 或运行状态放进 repo。
- `entrypoint.sh` 生成的 secrets 属于 `/data/config/generated.env`；不要提交或在 docs/AGENTS 粘贴真实 generated secret。
- `dify_demo_password`、`dify_redis_password`、`dify_ops_demo_token` 这类值只能作为 demo 默认值描述，不要写成生产安全建议。
- `HF_HOME` / `HF_HUB_CACHE` 默认落在 `${RUNTIME_ROOT}/hf-cache` 和 `${HF_HOME}/hub`，属于 runtime cache，不是 bucket-lite 核心持久状态。
- env 加载顺序是：Docker/HF 已注入 env 优先，`docker/dify.env.runtime` 用默认值补齐，`/data/config/generated.env` 补齐自动生成 secrets。不要破坏显式外部 env 覆盖。
- 新增或重命名 env var 时，同步检查 `docker/dify.env.runtime`、`docker/dify.env.demo`、相关 `docker/with-*` wrapper、`docker/entrypoint.sh`、`docker/supervisord.conf`、`docker/ops_service.py`、`docker/admin_service.py`和 `docs/configuration.md`。
...
- Nginx 路由修改必须保护 `/nginx-health`、`/healthz`、`/_ops/`、`/_admin/`、`/console/api`、`/api`、`/v1`、`/files`、`/mcp`、`/triggers`、`/socket.io/`、`/e/`、`/explore` 和 `/`，除非用户明确要求重做路由。
- `/socket.io/` 必须保留 WebSocket `Upgrade` / `Connection` header。
- `/e/` 必须保留 `Dify-Hook-Url`，用于 Plugin Daemon endpoint hook。
- `NGINX_PORT` 和 `NGINX_CLIENT_MAX_BODY_SIZE` 是 env defaults；除非实现模板渲染，不要声称它们会动态改变静态 `nginx.conf`。
...
- 排查 live 502 时按顺序看：HF runtime stage/SHA、`/nginx-health`、`/healthz`、`/_ops/health`、`/_ops/status`、`/_ops/errors`、定向 `/_ops/logs`、HF app/build logs。
...
- 不要在用户没有明确要求时推送 GitHub remote 或 Hugging Face remote；先确认要更新哪个 remote/branch。
...
wc -c AGENTS.md docker/AGENTS.md
rg -n "[[:blank:]]$" AGENTS.md docker/AGENTS.md
```
